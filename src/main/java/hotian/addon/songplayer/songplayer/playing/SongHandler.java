package hotian.addon.songplayer.songplayer.playing;

import hotian.addon.songplayer.songplayer.Config;
import hotian.addon.songplayer.songplayer.FakePlayerEntity;
import hotian.addon.songplayer.songplayer.SongPlayer;
import hotian.addon.songplayer.songplayer.Util;
import hotian.addon.songplayer.songplayer.mixin.ClientPlayerInteractionManagerAccessor;
import hotian.addon.songplayer.songplayer.song.*;
...
public class SongHandler {
    ...
    public void loadSong(String location) {
        if (loaderThread != null) {
            Util.showChatMessage("§c正在加载歌曲，无法加载另一个");
        }
    ...
    private void handleBuilding() {
        ...
        if (stage.nothingToBuild()) { // If there's still nothing to build after checking build status, switch to playing
            ...
            stage.sendMovementPacketToStagePosition();
            Util.showChatMessage("§6正在播放 §3" + currentSong.name);
        }
